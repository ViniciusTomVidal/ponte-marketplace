  ...
  <div class="bg-gray-50 min-h-screen">
    ...
    <div v-if="loading" class="fixed inset-0 bg-white bg-opacity-90 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-xl p-8 flex flex-col items-center min-w-[300px] border border-gray-200">
        ...
        <p class="text-gray-700 font-medium">Updating property...</p>
          ...
          <div class="space-y-4">
            <div id="mainImageContainer">
              ...
              <div v-if="form.mainImagePreview" class="mt-2">
                ...
                <p class="text-xs text-gray-500 mt-1">{{ form.mainImageFile?.name || 'Current image' }}</p>
              </div>
            </div>
  ...
  methods: {
    handleMainImage(event) {
      ...
      if (file) {
        ...
        // Quando uma nova imagem é selecionada, limpa a URL da imagem existente
        // (a nova imagem substitui a existente)
    ...
    handleAdditionalImages(event) {
      ...
      // Total limit is 5 additional images (existing URLs to keep + new files)
      // Usar existingAdditionalImageUrls para contar apenas as que serão mantidas
      const existingUrlCount = this.form.existingAdditionalImageUrls ? this.form.existingAdditionalImageUrls.length : 0
      const currentNewFilesCount = this.form.additionalImageFiles ? this.form.additionalImageFiles.length : 0
      const maxNewImages = Math.max(0, 5 - existingUrlCount)
      const filesToProcess = validFiles.slice(0, Math.max(0, maxNewImages - currentNewFilesCount))
      
      if (filesToProcess.length === 0) {
        if (validFiles.length > 0) {
          alert('Maximum of 5 additional images allowed. Please remove some existing images first.')
        }
        event.target.value = ''
        return
      }
      ...
      this.form.additionalImageFiles.push(...filesToProcess)
      
      // Create previews for new files and add to preview array
      filesToProcess.forEach((file) => {
        const reader = new FileReader()
        reader.onload = (e) => {
          // Add to the end of preview array (after existing URLs)
          this.form.additionalImagesPreview.push(e.target.result)
        }
        reader.readAsDataURL(file)
      })
      ...
      if (filesToProcess.length < validFiles.length) {
        alert(`Only ${filesToProcess.length} image(s) were added. Maximum of 5 additional images allowed.`)
      }
    ...
    removeAdditionalImage(index) {
      ...
      const preview = this.form.additionalImagesPreview[index]
      ...
      if (isExistingImage) {
        ...
        const urlIndex = this.form.existingAdditionalImageUrls.indexOf(imageUrl)
        if (urlIndex > -1) {
          this.form.existingAdditionalImageUrls.splice(urlIndex, 1)
        }
        this.form.additionalImagesPreview.splice(index, 1)
      } else {
        // It's a new image (data URI) - new images are added at the end
        // Count existing images (URLs) to find the index in the new images array
        // As novas imagens são sempre adicionadas após as existentes no preview
        const existingCount = this.form.existingAdditionalImageUrls ? this.form.existingAdditionalImageUrls.length : 0
        
        // Calculate the index in the new images array
        const newImageIndex = index - existingCount
        
        // Remove from files array if index is valid
        if (newImageIndex >= 0 && newImageIndex < this.form.additionalImageFiles.length) {
          this.form.additionalImageFiles.splice(newImageIndex, 1)
        }
      ...
      if (!this.validateForm()) {
        ...
        if (firstErrorField) {
          const errorElement = document.getElementById(
            ...
            firstErrorField === 'total_value' ? 'total_value' : firstErrorField
          )
          if (errorElement) {
