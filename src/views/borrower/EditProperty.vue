...
export default {
  ...
  mounted() {
    ...
    if (this.propertyId) {
      this.draftKey = `property_draft_borrower_edit_${this.propertyId}`
      this.loadProperty()
    } else {
    ...
    async loadProperty() {
      ...
      try {
        ...
        if (propertyData) {
          ...
          // Location - Parse address_line1 into street and number
          if (propertyData.address_line1) {
            const parts = propertyData.address_line1.split(',').map(p => p.trim())
            this.form.street = parts[0] || ''
            this.form.number = parts.slice(1).join(', ') || ''
          } else {
            this.form.street = ''
            this.form.number = ''
          }
      ...
      if (!this.validateForm()) {
        ...
        if (firstErrorField) {
          const errorElement = document.getElementById(
            ...
            firstErrorField === 'address_line1' ? 'street' :
